...
function makeHTML(glbRelPath) {
  const camSettings = glbRelPath.includes("cap")
    ? "const s2=new T.Box3().setFromObject(m);const ctr=s2.getSize(new T.Vector3());const maxD=Math.max(ctr.x,ctr.y,ctr.z);const dist=maxD*1.8;c.position.set(0,0.08,dist);c.lookAt(0,0.05,0);"
